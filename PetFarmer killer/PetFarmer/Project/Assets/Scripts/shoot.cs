		...
		if(WeaponManager.Weapon == 3)
		{
			if(Input.GetMouseButtonDown(0))
			{
				if(PlayerHealth.rifleAmmo > 0)
				{
					...
					particle.transform.position = akParticleLoc.transform.position;
					particle.transform.rotation = Camera.main.transform.rotation;
		...
		if(WeaponManager.Weapon == 1)
		{
			if(Input.GetMouseButtonDown(0))
			{
				if(PlayerHealth.rifleAmmo > 0)
				{
					...
					particle.transform.position = handgunParticleLoc.transform.position;
					particle.transform.rotation = Camera.main.transform.rotation;
		...
		if(WeaponManager.Weapon == 4)
		{
			if(Input.GetMouseButtonDown(0))
			{
				if(PlayerHealth.rocketAmmo > 0)
				{
					...
					particle.transform.position = rocketParticleLoc.transform.position;
					particle.transform.rotation = Camera.main.transform.rotation;

					GameObject projectile = Instantiate (rocketProjectile) as GameObject;
					projectile.transform.position = rocketParticleLoc.transform.position;
